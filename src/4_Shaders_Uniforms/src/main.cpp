...
int main()
{
    ...
    */
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        double timeValue = chronometer.GetTime();               //float timeValue = glfwGetTime();
        float greenValue = std::sin(timeValue) / 2.0f + 0.5f;
        int vertexColorId = glGetUniformLocation(shaderProgram, "ourColor");
        glUniform4f(vertexColorId, 0.0f, greenValue, 0.0f, 1.0f);

        //glDrawArrays(GL_TRIANGLES, 0, 6);         // use if there's no EBO
