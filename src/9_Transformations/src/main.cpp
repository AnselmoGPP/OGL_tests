...
int main()
{
    ...
    Shader myProgram(
                "../../../src/9_Transformations/shaders/vertexShader.vs",
                "../../../src/9_Transformations/shaders/fragmentShader.fs" );
    ...
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(float), (void *)nullptr);                // position attribute
    ...
    image = stbi_load("../../../src/8_Texture_units/textures/box1.jpg", &width, &height, &numberChannels, 0);
    if(image)
    {
    ...
    image = stbi_load("../../../src/9_Transformations/textures/note.png", &width, &height, &numberChannels, 0);
    if(image)
    {
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        transform = glm::rotate(transform, (float)chronometer.GetTime(), glm::vec3(0.0f, 0.0f, 1.0f));

        myProgram.UseProgram();

        glUniformMatrix4fv(glGetUniformLocation(myProgram.ID, "transform"), 1, GL_FALSE, glm::value_ptr(transform));
