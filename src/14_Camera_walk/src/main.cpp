...
int main()
{
    ...
    Shader myProgram(
                "../../../src/14_Camera_walk/shaders/vertexShader.vs",
                "../../../src/14_Camera_walk/shaders/fragmentShader.fs" );
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glm::mat4 projection = glm::perspective(glm::radians(fov), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f); // If it doesn't change each frame, it can be placed outside the render loop
...
void mouse_callback(GLFWwindow *window, double xpos, double ypos)
{
    ...
    float yoffset = (lastY - ypos) * sensitivity;   // reversed since y-coords go from bottom to top
    lastX = xpos;
    lastY = ypos;
